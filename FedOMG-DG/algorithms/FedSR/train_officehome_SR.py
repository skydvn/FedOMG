...
def get_argparse():
    ...
    parser.add_argument('--nbc', type=int, default=65)

    return parser.parse_args()
 
def main():
    '''log part'''
    file_name = 'fedsr_'+os.path.split(__file__)[1].replace('.py', '')
    args = get_argparse()
        
    if args.log:
        args.run_name = f"FedSR__{args.dataset}__test_domain_{args.test_domain}__{int(time.time())}"
        wandb.init(
            project="FL-DG",
            entity="ABCD",
            config=args,
