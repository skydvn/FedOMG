...
def main():
    ...
    args = get_argparse()
    if args.log:
        args.run_name = f"FedAvg__{args.dataset}__OMG__test_domain_{args.test_domain}__{int(time.time())}"
        wandb.init(
            project="FL-DG",
            entity="ABCD",
            config=args,
    ...
    global_model, model_dict, optimizer_dict, scheduler_dict = GetFedModel(args, args.num_classes, True)
    ...
    for i in range(args.comm+1):
        ...
        for domain_name in dataobj.train_domain_list:
            site_train(i, domain_name, args, model_dict[domain_name], optimizer_dict[domain_name], 
                       scheduler_dict[domain_name],dataloader_dict[domain_name]['train'], metric)
        ...
        global_model_dict = FedOMG(model_dict, weight_dict, global_model, dataobj, args.global_model_lr, args.parameter_c)
